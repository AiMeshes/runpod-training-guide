...
2. **实测（§2）：64M–1B 区间 RTX 4090 是最优解** —— 同时最快（H100 除外）和最便宜。
3. **小模型喂不满大卡**：H100 的 7 倍算力在 64M 上只换回 3.3 倍速度。**≤300M 不要用 H100**（§2）。
4. **≥3B 必须上 A100/H100**，因为优化器状态放不下 24GB。这是成本断崖。
5. **LoRA 省的是显存，不是算力**。前反向 FLOPs 与全参相同，只在优化器步省。
6. **后训练的成本在生成，不在训练**。PPO/GRPO 中 70–90% GPU 时间是 rollout。
7. **⚠️ Spot 和按需同价**（§2 实测）——**Spot 是纯负收益**，同样的钱白承担抢占风险。
   实测：Spot 跑 14 分钟被抢占，Pod 直接从 API 消失，进度全丢。**一次性训练用按需。**
8. **⚠️ 价格表上有 ≠ 能开出来**：A5000 标价 $0.16 但无货；部分机器驱动不支持 `cuda>=12.8`。
   选型要留备选（§2）。
9. **不需要 GitHub Actions**（见 §8.5）。唯一必须外部完成的事是"发现 Pod 被抢占"，本地 cron 或 $5 VPS 就够。
10. **不要在 Mac 本地 build 镜像**。Apple Silicon 编译 amd64 的 flash-attn 会走 qemu，极慢甚至失败。
11. **最大的两个钱坑**：volume disk 停止后涨到 $0.20/GB/月；忘记 terminate 的 Pod。
12. **先花 $0.5 校准**，再外推总成本。本文数字已有实测校准，但换模型/换数据后仍需重测。
...
上表的"有效 TFLOPS"是估算。2026-09 用真实 minimind 负载（64M 模型、
1.24GB 数据、batch 32）实测后，看到了**估算容易漏掉的一层**：

| 对比 | 算力倍数 | 实测速度倍数 | 缩放效率 |
|---|---|---|---|
| 3090 → 4090 | 2.32× | 2.00× | 86% |
| 4090 → H100 NVL | 3.00× | **1.67×** | **56%** |
| 3090 → H100 NVL | 6.96× | **3.33×** | **48%** |

**H100 的 7 倍算力只换回 3.3 倍速度。** 64M 模型每步计算量太小，
喂不满 H100 的 SM，瓶颈转移到数据加载和 kernel 启动开销。

反推的 MFU 更能说明问题 —— **卡越大，利用率越低**：

| GPU | 峰值 | 实测 steps/s | 反推 MFU |
|---|---|---|---|
| RTX 3090 | 142 | 6.13 | **18.0%** |
| RTX 4090 | 330 | 12.23 | **15.5%** |
| H100 NVL | 989 | 20.41 | **8.6%** |

> **判据不是"模型多大"，而是"每步多少工作量"**：
> `每步 FLOPs ≈ 6 × N × (batch × seq)`。
> 要让 H100 跑到 ~40% MFU 需要每步约 40 TFLOPs 以上 ——
> 换算下来大致是 **1B 起步**（或小模型配很大的 batch）。
>
> **≤300M 不要用 H100。** 买它的算力，用不到一半。

完整实测数据和可复现脚本见
[minimind-runpod/BENCHMARKS.md](https://github.com/AiMeshes/minimind-runpod/blob/main/BENCHMARKS.md)。
...
| 约束 | 表现 |
|---|---|
| **A5000 在售但无货** | 价格 $0.16 能查到，创建时返回 "no instances available" |
| **机器驱动版本不一致** | 部分机器驱动不支持 `cuda>=12.8`，误分配到就容器起不来，且 RunPod 无限重试 |
| **挂卷大幅收窄可用机器** | 18 个支持卷的机房 × 4090/3090 × Spot/按需，36 种组合全部无货（见 §11.8） |

> 第一条的实践含义：**"能查到价格"和"能真的开出来"是两回事**。
> 选型时要留备选，别把方案押在一款卡上。
...
查 RunPod API 的 `lowestPrice`，**每一款 GPU 的 `minimumBidPrice` 与
`uninterruptablePrice` 完全相等**：
...
**这意味着 Spot 是纯负收益** —— 同样的价格承担被抢占的风险。
实测验证：一轮 Spot 跑到 14 分钟被抢占，Pod 直接从 API 消失（404），
容器盘连同全部进度丢失。

> **对一次性训练直接用按需。** 需要 Spot 的前提是有 network volume
> 能接续 —— 而挂卷的机器当前很难开出来（§11.8）。
...
### 11.5 冷启动成本：Spot 决策的第三个变量

**实测数据**（2026-09）：10.6GB 官方镜像在 Community 机器上**冷拉耗时约 11 分钟**，
费用 $0.06。若该机器已有镜像缓存则几乎瞬时。

**这修正了 §7.1 的决策阈值** —— 原先只比单价，但冷启动时间也是钱：

| 场景 | Community 冷拉 | 每次重建的额外成本 |
|---|---|---|
| 短任务（< 30 分钟） | +11 分钟 | **接近 Secure 溢价的一半** |
| 长训练（数小时～数天） | +11 分钟（一次性） | 可忽略 |

> **判据**：任务时长 < 1 小时时，把冷启动算进去后再决定用不用 Spot。
> 对 §3 里那些 $1,000+ 的长训练，冷拉 11 分钟完全无所谓。

**避免重复冷拉**：RunPod 的 `PodUpdateInput` 支持原地改 `dockerStartCmd` / `env` /
`imageName`，所以改启动脚本后用 `PATCH /pods/{id}` + 重启，**复用同一台机器的镜像**。
「终止 + 重建」会换机器，缓存全部作废。

### 11.6 修正：容器崩溃 vs 被抢占，恢复路径不同

原先的认知"容器死了必须外部重启"**不准确**。实测发现两种情形性质完全不同：

| 情形 | Pod `desiredStatus` | 恢复方式 |
|---|---|---|
| **进程死亡**（训练崩溃 / OOM） | 保持 `RUNNING` | ⭐ **RunPod 自动重启容器**，supervisor 自己从 checkpoint 接续 |
| **RunPod 抢占** | 转 `EXITED` | **不会**自动重启 —— 需外部监控拉起 |

实测证据：SIGKILL 掉训练进程组后，Pod 保持 RUNNING，容器被自动重启，
日志出现 `跳过前1100个step，从step 1101开始` —— 从 checkpoint 精确接续。

**所以 §4.3 的 supervisor 加上外部监控，覆盖的是两个不同的失败模式**，
缺一不可：supervisor 管进程崩溃，外部监控管真正的抢占。

### 11.7 实证：容器盘确实在 stop 时被擦除（§5 的断言已验证）

§5 的"容器盘停止即擦除"此前只有文档依据，2026-09 用真实运行验证：

**测试**：训练跑到 checkpoint step 200 → `POST /pods/{id}/stop`
（产生 `desiredStatus=EXITED`，与真抢占一致）→ 外部监控拉起。

| 项 | 结果 |
|---|---|
| 监控能否检测并拉起 | ✓ 正确检测 `EXITED` 并 start |
| **重启后容器盘内容** | ✗ **全空** —— 代码 / 1.24GB 数据 / 649MB checkpoint 全部消失 |
| 训练起点 | 从 step 0 重来 |

> **关键推论：没有 network volume 时，抢占恢复机制虽然正确但毫无意义**
> —— 恢复后从 step 0 重来，而不是从 checkpoint 接续。
>
> `supervisor` 和外部监控保证的是「进程被重新拉起」；
> 只有 **network volume** 保证「拉起来之后还能接上」。两者是不同的事。

### 11.8 ⚠️ 挂 network volume 会大幅收窄可用机器

实测（2026-09）：**18 个支持卷的机房 × 4090/3090 × Spot/按需，共 36 种组合全部无货**，
报错 `There are no longer any instances available`。
而**同一批机房不带卷时全部都能开**（且是同一时段）。

原因：挂卷要求 Pod 落在既能挂载该卷、又有对应 GPU 的机器上，符合条件的机器少得多。

**另一个相关问题**：网络卷**只在 18 个机房支持**，而 Pod 能开在 28 个机房 ——
两个列表不一致，让 §9.6 的地域锁定问题更严重。

| 约束 | 影响 |
|---|---|
| 卷只能建在 18 个机房（Pod 可开的 28 个之中） | 选择面先砍掉 1/3 |
| 建了卷后 Pod 只能在该机房 | 无法借其他机房的现货 |
| 挂卷进一步收窄可用机器 | 实测 36 种组合全部无货 |

> **实践建议**：需要 volume 的长时间训练，**先把 Pod 开起来确认现货充足，
> 再决定是否迁移**；或者接受 `EXITED` 后等待容量恢复（`watch.py` 会持续重试）。
> 短期内做实验性训练，**不挂卷 + 随时可重建** 反而是更务实的选择。
